...
/* in-order thread binary search tree */
void in_thread(tNode* root);
void pre_thread(tNode* root);
void post_thread(tNode* root);
...
/* thread make us add more parameter, previous sequence node */
void in_thread(tNode* root, tNode* pre) {
    if(root == NULL) {
        return;
    }
    ...
    if(pre != NULL) { 
        if(pre->right == NULL) {
            ...
            pre->right_flag = child;
        }
    } 
    /* set current node's previous */
    if(root->left == NULL) {
        root->left = pre;
        root->left_flag = thread;
    } else {
        /* otherwise set the child flag */
        root->left_flag = child;
    }
    pre = root;
